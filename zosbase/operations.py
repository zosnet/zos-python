    ...
    def __init__(self, *args, **kwargs):
        ...
        if isArgsThisClass(self, args):
            ...
            super().__init__(OrderedDict([
                ...
                ('limitactive', Permission(kwargs["limitactive"], prefix=prefix)),# zos chain add
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isArgsThisClass(self, args):
            ...
            if "limitactive" in kwargs:
                limitactive = Optional(Permission(kwargs["limitactive"], prefix=prefix))
            else:
...
class Account_coupon(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
                self.data = args[0].data
        else:
...
class Bitlender_loan(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            self.data = args[0].data
        else:
            ...
            super().__init__(OrderedDict([
                ('fee', Asset(kwargs["fee"])),
                ('issuer', ObjectId(kwargs["issuer"], "account")),
                ('carrier', ObjectId(kwargs["carrier"], "account")),
                ('order', String(kwargs["order"])),
                ('amount_to_loan', Asset(kwargs["amount_to_loan"])),
                ('loan_period', Uint32(kwargs["loan_period"])),
                ('interest_rate', Uint32(kwargs["interest_rate"])),
                ('repayment_type', Uint32(kwargs["repayment_type"])),
                ('amount_to_collateralize', Asset(kwargs["amount_to_collateralize"])),
                ('collateral_rate', Uint32(kwargs["collateral_rate"])),
                ('bid_period', Uint32(kwargs["bid_period"])),
                ('memo', String(kwargs["memo"])),
                ('extensions', Set([])),
            ]))


class Bitlender_invest(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            self.data = args[0].data
        else:
            ...
            super().__init__(OrderedDict([
                ('fee', Asset(kwargs["fee"])),
                ('issuer', ObjectId(kwargs["issuer"], "account")),
                ('loan_id', ObjectId(kwargs["loan_id"], "account")),
                ('carrier', ObjectId(kwargs["carrier"], "account")),
                ('order_id', ObjectId(kwargs["order_id"], "bitlender_order")),
                ('amount_to_invest', Asset(kwargs["amount_to_invest"])),
                ('extensions', Set([])),
            ]))

class Bitlender_repay_interest(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            self.data = args[0].data
        else:
            ...
            super().__init__(OrderedDict([
                ...
                ('issuer', ObjectId(kwargs["issuer"], "account")),
                ('order_id', ObjectId(kwargs["order_id"], "bitlender_order")),
                ('repay_period', Uint32(kwargs["repay_period"])),
            ]))

class Account_property(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            self.data = args[0].data
        else:
            ...
            super().__init__(OrderedDict([
                ('fee', Asset(kwargs["fee"])),
                ('issuer', ObjectId(kwargs["issuer"], "account")),
                ('uaccount_property', Uint32(kwargs["uaccount_property"])),
                ('uaccount_mask', Uint32(kwargs["uaccount_mask"])),
            ]))

class Gateway_withdraw(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            self.data = args[0].data
        else:
            ...
            super().__init__(OrderedDict([
                ...
                ('from', ObjectId(kwargs["from"], "account")),
                ('to', ObjectId(kwargs["to"], "account")),
                ('withdraw', Asset(kwargs["withdraw"])),
            ]))

class Gateway_deposit(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            self.data = args[0].data
        else:
...
class Gateway_issue_currency(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
            self.data = args[0].data
        else:
            ...
            super().__init__(OrderedDict([
                ...
                ('issuer', ObjectId(kwargs["issuer"], "account")),
                ('account', ObjectId(kwargs["account"], "account")),
                ('issue_currency', Asset(kwargs["issue_currency"])),
            ]))

class Bitlender_publish_feed(GrapheneObject):
    def __init__(self, *args, **kwargs):
        if isArgsThisClass(self, args):
                self.data = args[0].data
        else:
