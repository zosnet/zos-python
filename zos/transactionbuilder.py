...
from zosbase.objects import Operation
from zosbase.account import PrivateKey, PublicKey
from zosbase.signedtransactions import Signed_Transaction
...
class TransactionBuilder(dict):
            ...
            for authority in account[perm]["key_auths"]:
                try:
                    wif = self.transnet.wallet.getPrivateKeyForPublicKey(
                        authority[0])
                    if wif is not None:#没判断为None会导致“grapheneapi.exceptions.RPCError: irrelevant signature included: Unnecessary signature(s) detected” @Chaimyu 2018.8.21
    ...
    def constructTx(self):
        ...
        # expiration = transactions.formatTimeFromNow(random.randint(10*self.transnet.expiration, 100*self.transnet.expiration))# 测试时快速转帐有同样交易数据
