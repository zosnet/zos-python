...
from zosapi.transnetnoderpc import TransnetNodeRPC
from zosbase.account import PrivateKey, PublicKey
from zosbase import transactions, operations
...
from .bitlender_order import Bitlender_Order
...
class Transnet(object):
    ...
    def connect(self,
        ...
        # 为什么没根据链设置前缀呢
        operations.default_prefix = self.prefix
    ...
    def create_account(
        ...
        **kwargs
        ...
        """ Create new account on Transnet
            ...
            (see `zosbase.account` for more details.
        ...
        if password and (owner_key or active_key or memo_key or auth_key):
            raise ValueError(
        ...
        auth = format(auth_pubkey, self.prefix)
    ...
    def coupon_account(self, account=None, **kwargs):
        """ Upgrade an account to Lifetime membership
    ...
    def publish_price_feed(
        ...
        op = operations.Asset_publish_feed(**{
            ...
            "feed": {
                ...
                "maintenance_collateral_cash_ratio": int(mccr * 10),
                "maximum_short_squeeze_cash_ratio": int(msscr * 10),
        ...
        """ Reserve/Burn an amount of this shares
            ...
            **Optional**
            ...
            :param str account: (optional) the account to allow access
                to (defaults to ``default_account``)
        """
        from zosbase.transactions import timeformat
    ...
    def bitlender_loan(self, issuer, params, carrier = "1.2.19", **kwargs):
        account = Account(issuer, transnet_instance=self)
        ...
        data.update(params)
        op = operations.Bitlender_loan(**data)
        return self.finalizeOp(op, account, "active", **kwargs)

    def account_coupon(self, issuer, **kwargs):
        account = Account(issuer, transnet_instance=self)
        ...
        op = operations.Account_coupon(**data)
        return self.finalizeOp(op, account, "active", **kwargs)

    def bitlender_invest(self, issuer, params, carrier = "1.2.19", **kwargs):
        account = Account(issuer, transnet_instance=self)

        order = Bitlender_Order(params["order_id"], transnet_instance=self)

        data = {
            "fee": {"amount": 0, "asset_id": "1.3.0"},
            "issuer": account["id"],
            "carrier": carrier,
            "loan_id": order["issuer"]
            }
        data.update(params)
        op = operations.Bitlender_invest(**data)
        return self.finalizeOp(op, account, "active", **kwargs)

    def bitlender_repay_interest(self, issuer, order_id, repay_period, **kwargs):
        account = Account(issuer, transnet_instance=self)
        ...
        op = operations.Bitlender_repay_interest(**data)
        return self.finalizeOp(op, account, "active", **kwargs)

    def set_proposal_transaction(self, **kwargs):
        gp = self.get_global_properties()
        self.proposal_review = gp["parameters"]["committee_proposal_review_period"]
        self.proposal_expiration = gp["parameters"]["maximum_proposal_lifetime"] - 100
        if kwargs.__contains__("proposer"):
            self.proposer = kwargs["proposer"]
            return True
        else:
            return False

    def clear_proposal_transaction(self):
        self.proposer = None
        self.proposal_review = 0
        self.proposal_expiration = 0

    def create_gateway_member(self, issuer, enable, **kwargs):
        account = Account(issuer, transnet_instance=self)

#define  account_gateway          0x00000001
#define  account_carrieroperator  0x00000002
#define  account_genesis          0x00000004 
        if enable:
            uaccount_property = 0x00000001
        else:
            uaccount_property = 0
        data = {
            "fee": {"amount": 0, "asset_id": "1.3.0"},
            ...
            "uaccount_mask": 0x00000001
            }

        op = operations.Account_property(**data)

        if not self.set_proposal_transaction(**kwargs):
            self.proposer = issuer
        ret = self.finalizeOp(op, account, "active", **kwargs)
        self.clear_proposal_transaction()

        return ret

    def gateway_withdraw(self, accountFrom, to, amount, symbol, **kwargs):
        account = Account(accountFrom, transnet_instance=self)
        accountTo = Account(to, transnet_instance=self)
        ...
        data = {
            ...
            "from": account["id"],
            "to": accountTo["id"],
            "withdraw": {"amount": amount, "asset_id": asset["id"]},
            }
        op = operations.Gateway_withdraw(**data)
        return self.finalizeOp(op, account, "active", **kwargs)

    def gateway_deposit(self, gateway, to, amount, symbol, **kwargs):
        account = Account(gateway, transnet_instance=self)
        accountTo = Account(to, transnet_instance=self)
        ...
        data = {
            ...
            "from": account["id"],
            "to": accountTo["id"],
            "deposit": {"amount": amount, "asset_id": asset["id"]},
            }
        op = operations.Gateway_deposit(**data)
        return self.finalizeOp(op, account, "active", **kwargs)

    def get_global_properties(self):
        return self.rpc.get_global_properties()

    def gateway_issue_currency(self, issuer, gateway, amount, symbol, **kwargs):
        account = Account(issuer, transnet_instance=self)
        accountGateway = Account(gateway, transnet_instance=self)
        asset = Asset(symbol, transnet_instance=self)

        data = {
            "fee": {"amount": 0, "asset_id": "1.3.0"},
            "issuer": account["id"],
            "account": accountGateway["id"],
            "issue_currency": { "amount": amount, "asset_id": asset["id"]}
            }

        op = operations.Gateway_issue_currency(**data)
    ...
    def bitlender_publish_price_feed(
        self,
        symbol,
        settlement_price,
        cer=None,
        ...
        op = operations.Bitlender_publish_feed(**{
            "fee": {"amount": 0, "asset_id": "1.3.0"},
            "publisher": account["id"],
            "asset_id": asset["id"],
            "feed": {
