    ...
    def __init__(
        ...
        *args,
        ...
        if (len(args) == 1 and isinstance(args[0], str) and not base and not quote):
            ...
            self["quote"] = Amount(amount=d, asset=quote, transnet_instance=self.transnet)
            self["base"] = Amount(amount=n, asset=base, transnet_instance=self.transnet)
            # self["quote"] = Amount(amount=frac.denominator, asset=quote, transnet_instance=self.transnet)
            # self["base"] = Amount(amount=frac.numerator, asset=base, transnet_instance=self.transnet)
    ...
    def __init__(self, feed, transnet_instance=None):
        ...
        if len(feed) == 2:
            super(PriceFeed, self).__init__({
                ...
                "maintenance_collateral_cash_ratio": feed[1][1]["maintenance_collateral_cash_ratio"],
                "maximum_short_squeeze_cash_ratio": feed[1][1]["maximum_short_squeeze_cash_ratio"]
            })
        else:
            super(PriceFeed, self).__init__({
                ...
                "maintenance_collateral_cash_ratio": feed["maintenance_collateral_cash_ratio"],
                "maximum_short_squeeze_cash_ratio": feed["maximum_short_squeeze_cash_ratio"]
            })
